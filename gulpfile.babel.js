...
gulp.task('styles:wp', () => {
	...
	return gulp.src([
		...
		.pipe($.sourcemaps.write('./'))
		.pipe(gulp.dest('/Applications/MAMP/htdocs/assa-abloy/wp-content/themes/assa-abloy-proyectos/dist/css'));
});
...
gulp.task('styles:sftp', ['styles'], () => {
	return gulp.src(app_path + '/dist/css/**')
		.pipe(sftp({
			...
			'remotePath': '/var/www/develop/network-idastage/wp-content/themes/ida-ketchup/dist/css/'
		}));
});
...
gulp.task('scripts:sftp', ['scripts'], () => {
	return gulp.src(app_path + '/dist/js/**')
		.pipe(sftp({
			...
			'user': 'ubuntu',
			'keyLocation': '~/.ssh/idamazon.pem',
			'remotePath': '/var/www/develop/network-idastage/wp-content/themes/ida-ketchup/dist/js/'
		}));
});
